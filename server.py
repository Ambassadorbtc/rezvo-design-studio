...
async def export_svg(html: str = Form(...), name: str = Form("export"), device: str = Form("desktop")):
    """Export scanned HTML as a layered SVG that Figma can import with proper structure."""
    w, h = DEVICE_SIZES.get(device, (1440, 900))
    
    # Extract the background image from scanner HTML
    bg_match = re.search(r"background-image:\s*url\('(data:image/[^']+)'\)", html)
    bg_data_url = bg_match.group(1) if bg_match else ""
    
    # Extract hotspot elements
    hotspots = re.findall(
        r'<div class="hotspot"[^>]*style="left:([^%]+)%;top:([^%]+)%;width:([^%]+)%;height:([^%]+)%[^"]*"[^>]*data-label="([^"]*)"[^>]*data-type="([^"]*)"',
        html
    )
    ...
    # Layer 1: Screenshot image (background)
    if bg_data_url:
        svg_elements.append(f'''  <g id="screenshot">
    <image href="{bg_data_url}" x="0" y="0" width="{w}" height="{h}" preserveAspectRatio="xMidYMid meet"/>
  </g>''')
    
    # Layer 2: Interactive elements as labeled rectangles
    if hotspots:
        hotspot_rects = []
        for left, top, width, height, label, el_type in hotspots:
            x = float(left) / 100 * w
            y = float(top) / 100 * h
            rw = float(width) / 100 * w
            rh = float(height) / 100 * h
            safe_label = label.replace('&', '&amp;').replace('<', '&lt;').replace('"', '&quot;')
            safe_id = re.sub(r'[^a-zA-Z0-9_-]', '_', label)[:30]
            
            # Color-code by type
            colors = {
                'button': '#3b82f6', 'card': '#8b5cf6', 'nav-item': '#10b981',
                'tab': '#f59e0b', 'link': '#ec4899', 'input': '#6366f1',
            }
            color = colors.get(el_type, '#3b82f6')
            
            hotspot_rects.append(f'''    <g id="{safe_id}">
      <rect x="{x:.1f}" y="{y:.1f}" width="{rw:.1f}" height="{rh:.1f}" 
            fill="{color}" fill-opacity="0.08" stroke="{color}" stroke-width="1.5" stroke-opacity="0.4" rx="6"/>
      <text x="{x + rw/2:.1f}" y="{y + rh/2 + 4:.1f}" text-anchor="middle" font-size="11" 
            font-family="-apple-system, BlinkMacSystemFont, sans-serif" fill="{color}" fill-opacity="0.7">{safe_label}</text>
    </g>''')
        
        svg_elements.append(f'  <g id="interactive-elements">\n' + '\n'.join(hotspot_rects) + '\n  </g>')
    
    svg = f'''<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
     width="{w}" height="{h}" viewBox="0 0 {w} {h}">
  <title>{name}</title>
  <desc>Generated by Rezvo Design Studio — Scanner Mode</desc>
...
async def export_png(html: str = Form(...), name: str = Form("export"), device: str = Form("desktop")):
    ...
    # Fallback: try any base64 image
    any_match = re.search(r"data:image/[^;]+;base64,([A-Za-z0-9+/=]+)", html)
    if any_match:
        img_bytes = base64.b64decode(any_match.group(1))
        return HTMLResponse(content=img_bytes, media_type="image/png",
                           headers={"Content-Disposition": f'attachment; filename="{name}.png"'})
    
    raise HTTPException(400, "No image found in HTML to export as PNG")
